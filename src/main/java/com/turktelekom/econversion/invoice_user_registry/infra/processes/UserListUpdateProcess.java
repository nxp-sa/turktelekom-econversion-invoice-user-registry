package com.turktelekom.econversion.invoice_user_registry.infra.processes;
...
public class UserListUpdateProcess {
    ...
    public void run() {
        ...
        log.info("User-List File downloading ..");
        ...
        if (optUserListFile.isEmpty()) {
            log.warn("User-List File download failed!");
            return;
        }

        var userListFile = optUserListFile.get();

        log.info("User-List File download completed -> {}",
                String.format("%s (%,d bytes)", userListFile.getChecksumId(), userListFile.getContentSize()));

        log.info("User-List File processing ..");

        var processUserListCommand = new ProcessUserListCommand(userListFile);
        var affected = sender.send(processUserListCommand);

        log.info("User-List Update process completed {}",
                affected ? String.format("with changes. %s (%,d users)", userListFile.getChecksumId(), userListFile.getUsersCount())
                        : "without changes.");
    }
}
